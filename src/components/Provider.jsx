...
const Provider = ({ children }) => {
  ...
  for (let i = 0; i < cartProducts.length; i++) {
    if (cartProducts[i].offer) {
      let discountedPrice =
        parseInt(cartProducts[i].publicPrice) -
        (cartProducts[i].discount / 100) *
          parseInt(cartProducts[i].publicPrice);
      totalPrice += discountedPrice * cartProducts[i].cantidad;
    } else {
      totalPrice +=
        parseInt(cartProducts[i].publicPrice) * cartProducts[i].cantidad;
    }
  ...
  const addToCart = (product) => {
    setCartProducts((prev) => {
      ...
      let existingProduct = updatedCart.findIndex(
      ...
      if (existingProduct !== -1) {
        updatedCart[existingProduct] = {
          ...updatedCart[existingProduct],
          cantidad: updatedCart[existingProduct].cantidad + 1,
        };
      } else {
