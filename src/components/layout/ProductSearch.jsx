...
const ProductSearch = ({ product, setSearch }) => {
  ...
  return (
    <div
      className="flex items-center mb-4 sm:m-0 hover:bg-blue-100 cursor-pointer"
      onClick={() => {
      ...
      <div className="grow text-end">
        ...
        {product.offer ? (
          <div className="flex flex-col md:flex-row md:gap-2 items-center justify-end mr-2">
            <p className=" text-sm self-end text-gray-500 font-bold">
              <del>${parseInt(product.price).toLocaleString("AR")}</del>
            </p>
            <p className=" font-bold self-end text-red-500">
              $
              {(
                parseInt(product.price) -
                (product.discount / 100) * parseInt(product.price)
              ).toLocaleString("AR")}
            </p>
          </div>
        ) : (
          <p className=" mr-2 text-red-500 font-semibold text-sm">
