...
const GraphicUsers = ({users}) => {
    ...
    users.forEach(user => {
        ...
        let totalCharged = user.purchasesMade.reduce((total, purchase) => total + purchase.total, 0)

        //Capturo los datos en el objeto
        data = {name: user.name,
               invoiced: totalCharged }
    ...
    const billingAndUsersOrdered = billingAndUsers.sort((a, b) => b.invoiced - a.invoiced)
  ...
  return (
     <div className='w-[80%] md:w-[30%] mx-auto mt-10 md:mt-0'>
        <h2 className='text-center text-xl font-bold'>Usuarios con mas compras</h2>
