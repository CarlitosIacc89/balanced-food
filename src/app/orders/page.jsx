...
const OrdersPage = () => {
  ...
  return (
    <section className="mt-8 min-h-[46vh]">
      ...
      {errorOrders && <ErrorUser errorText={text} setErrorText={setText} />}
      {loader && (
      ...
      {deleteConfirm && (
        <div className="inset-0 fixed bg-black/80 flex items-center justify-center z-10">
          <div className="flex flex-col w-80 lg:w-96 bg-white rounded-lg  items-center">
            <span
              className="self-end pt-2 px-2 cursor-pointer"
              onClick={() => setDeleteConfirm(false)}
            >
              <Close className="w-7 h-7" />
            </span>
            <p
              className={`flex flex-col gap-4 items-center text-xl text-gray-500 font-bold mb-8`}
            >
      ...
      {orders.length > 0 &&
        orders.map((order) => (
          <div
            key={order._id}
            className="relative bg-white p-4 my-12 max-w-4xl mx-auto text-center confirm"
          >
            <fieldset className="border relative">
              <legend className="text-sm sm:text-xl md:text-2xl font-bold text-green-400">
                Orden id: {order._id}
              </legend>
              ...
              <table className="w-full">
                <thead className="bg-gray-400">
                  <tr>
                    <th className="md:w-[250px]">ARTICULO</th>
                    <th>CANTIDAD</th>
                    <th>PRECIO</th>
                  </tr>
                </thead>
                <tbody>
                  {order.sale.map((pro) => (
                    <tr key={pro._id} className="border row">
                      <td>{pro.title}</td>
                      <td>{pro.cantidad}</td>
                      <td>
                        $
                        {pro.offer
                          ? (
                              (parseInt(pro.publicPrice) -
                                (pro.discount / 100) *
                                  parseInt(pro.publicPrice)) *
                              pro.cantidad
                            ).toLocaleString("AR")
                          : (
                              parseInt(pro.publicPrice) * pro.cantidad
                            ).toLocaleString("AR")}
                      </td>
                    </tr>
                  ))}
              ...
              <div className="flex justify-evenly p-2 bg-gray-300">
                <div className="font-bold">
                  Cantidad de Productos: {order.totalProducts}
                </div>
                <div className="font-bold">
                  Total a pagar: {formatter.format(order.totalPrice)}
                </div>
              </div>
            </fieldset>
            <button
              type="button"
              className="bg-red-500 mx-auto my-4"
              onClick={() => {
