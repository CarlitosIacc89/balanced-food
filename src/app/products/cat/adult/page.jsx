...
async function getData(){
    try {
        const res = await fetch(`http://localhost:3000/api/products?stage=adult&specie=cat`, {cache:"no-store"})
        if(!res.ok){
...
const CatAdultPage = async () => {
  ...
  return (
    <section className='mt-8'>
        <SectionHeaderFood img={"/Gatos-adultos.jpg"} text={"Alimentos para gatos adultos"} items={items}/>
          {items.length > 0 ? (
