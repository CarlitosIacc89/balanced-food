...
async function getData(){
    try {
        const res = await fetch(`http://localhost:3000/api/products?specific=true&specie=cat`, {cache:"no-store"})
        if(!res.ok){
...
const PageCatSpecific = async () => {
    const items = await getData()

  return (
    <section className='mt-8'>
        <SectionHeaderFood img={"/Especificos-gatos.jpg"} text={"Alimento especificos para gatos"} items={items}/>
          {items.length > 0 ? (
...
export default PageCatSpecific