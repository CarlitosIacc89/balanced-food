...
async function getData(){
    try {
        const res = await fetch(`http://localhost:3000/api/products?specific=true&specie=dog`, {cache:"no-store"})
        if(!res.ok){
...
const PageDogSpecific = async () => {
  ...
  return (
    <section className='mt-8'>
        <SectionHeaderFood img={"/Perros-especificos.jpg"} text={"Alimentos especificos para perros"} items={items}/>
         {items.length > 0 ? (
