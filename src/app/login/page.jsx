...
const LoginPage = () => {
  ...
  return (
    <section className="mt-4 md:pb-8 ">
      {error && <ErrorUser errorText={error} setErrorText={setError} />}
      {success && (
        <SuccessUser
          successText={success}
          setSuccessText={setSuccess}
          text={"¡¡Inicio de sesion exitoso!!"}
        />
      )}
      <form className="max-w-lg mx-auto" onSubmit={handleSubmit}>
        <fieldset className="border rounded-md text-center flex flex-col p-4 pt-4 gap-4 bg-white">
          <legend className="text-3xl px-4 font-bold text-red-500 ">
            Iniciar Sesion
          </legend>
          <input
            type="email"
          ...
          <div className="relative">
            <div
              className="absolute inset-y-0 right-1 top-1 p-1 px-2 cursor-pointer"
              onClick={() => setEye(!eye)}
            >
              {eye ? <Eye /> : <CloseEye />}
            </div>
          ...
          <button
            ...
            className="flex gap-4 justify-center mb-2 text-gray-700 border"
          >
          ...
          <div className="text-gray-400 font-bold">
            ...
            <Link className="underline text-gray-800" href={"/register"}>
              Registrate
            </Link>
          </div>
        </fieldset>
